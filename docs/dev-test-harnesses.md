...
| `pnpm bridge-test`                          | headless Chromium   | In-game bridge end to end: virtual storage (isolation, save, relaunch, fresh origin, postMessage pull, nested same-origin frames, slow profile store, forged `hydrate`), typed IndexedDB, live key detection, Controls menu (purposes, search, grouping), dynamic console buttons, shortcut / typing classification, single dispatch per press, Move mode |

`pnpm bridge-test` starts its own Vite dev server (or reuses one: `-- --url http://localhost:5173`), writes fixture games to `static/games/_bridge-*` and removes them afterwards. It keeps profiles in the browser's IndexedDB by refusing the puller's health and browser-data routes, so a puller running on the machine cannot change the outcome. `-- --out <dir>` saves a screenshot of each stage; `CHROMIUM_PATH` picks the browser binary.
