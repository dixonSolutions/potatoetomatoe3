...
- The stores are ready **synchronously** at boot, from the first of:
  ...
  3. a `pull` over postMessage (cross-origin frames, e.g. the puller on loopback). If that
     ...
     the profile across the reload so it wins over anything written in between.
- Changes are pushed back (debounced, and on pause / `pagehide`) only once the saved
  profile is known, so an empty boot can never overwrite real saves.
...
exception is the reload-onto-late-profile case above. `UnityCache` (a rebuildable asset
cache) is not mirrored.
