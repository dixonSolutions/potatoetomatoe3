...
<div
	class={`relative w-full overflow-hidden rounded-lg border bg-gradient-to-br from-slate-900 to-slate-800 ${
		orientation === 'portrait' ? 'mx-auto aspect-[3/4] max-w-64' : 'aspect-video'
	}`}
	role="presentation"
...
>
	...
	>
		<div
			class="absolute top-2 left-1/2 flex h-7 w-14 -translate-x-1/2 items-center justify-center rounded-full border border-white/25 bg-white/10 text-white/80"
		>
	...
	<div
		class="pointer-events-none absolute top-3 right-3 flex items-center gap-2 rounded-xl border border-white/25 bg-background/70 px-3 py-2 text-foreground shadow-md backdrop-blur-md"
	>
		<Gamepad2 class="size-4 text-blue-500" aria-hidden="true" />
		<Switch
			checked={true}
			class="h-6 w-11 data-[state=checked]:bg-blue-500"
			aria-label="Touch controls preview"
		/>
	</div>
</div>
