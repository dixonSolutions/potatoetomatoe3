...
export async function loadGameIds(): Promise<string[]> {
	...
	if (
		cachedGameIds &&
		((mtime != null && mtime === cachedListMtimeMs) || (mtime == null && cachedListMtimeMs == null))
	) {
...
/** Offline root that contains a complete mirror (entry file), preferring user data over catalog. */
export async function resolveOfflineMirrorRoot(gameId: string): Promise<string | null> {
	const entryRel = await resolveOfflineEntryRel(gameId);
...
export function resolveOfflineFilePath(gameId: string, fileRel: string): string | null {
	...
	for (const candidate of candidates) {
		...
		const allowedRoots = [path.resolve(GAMES_DATA_DIR), path.resolve(CATALOG_DIR)];
